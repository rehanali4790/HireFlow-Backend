...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.error('List job departments error:', error);
  ...
  async (req, res) => {
    ...
    try {
      const existing = await db.query(
        `SELECT id, name FROM job_departments
      ...
      const result = await db.query(
        `INSERT INTO job_departments (employer_id, name, created_at, updated_at)
         VALUES ($1, $2, NOW(), NOW())
      ...
      console.error('Create job department error:', error);
